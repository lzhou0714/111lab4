...
#define NUM_BLOCKS 1024 //1024 1kB blocks
#define NUM_INODES 128 //can support up to 128 file in file system

#define LOST_AND_FOUND_INO 11 //start at 11, first 10 inodes are reserved in ext2
...
#define EXT2_SUPER_MAGIC 0xEF53   //from documentation
#define MAX_MT_COUNT -1
#define CHECK_INTERVAL 1

#define NUM_FREE_BLOCKS (NUM_BLOCKS - LAST_BLOCK - 1)
#define NUM_FREE_INODES (NUM_INODES - LAST_INO)
#define NUM_DIR_USED 2

/* http://www.nongnu.org/ext2-doc/ext2.html */
/* http://www.science.smith.edu/~nhowe/262/oldlabs/ext2.html */

//ext2 defaults
#define	EXT2_BAD_INO             1 //1st inode is bad
#define EXT2_ROOT_INO            2 //2 inode is root
#define EXT2_GOOD_OLD_FIRST_INO 11 //start at 11, first 10 inodes are reserved in ext2
...
void write_superblock(int fd) {
	off_t off = lseek(fd, BLOCK_OFFSET(1), SEEK_SET); //move posituion to 1kB in file
	if (off == -1) {
		errno_exit("lseek");
	}
	...
	superblock.s_r_blocks_count    = 0; //we have no reserved blocks
	...
	superblock.s_first_data_block  = SUPERBLOCK_BLOCKNO; /* First Data Block */ 
		//since our blocksize is 1Mb (1024), our superblock is at 1
	superblock.s_log_block_size    = 0; // block size = 2^(this number +10) leavs as 0, since 2^(0+10) = 1024
	superblock.s_log_frag_size     = 0; /* 1024 assume = block size*/ 
	//total number of blocks that can be represented in a group (1 byte on bitmap?), assume block = frags
	superblock.s_blocks_per_group  = BLOCK_SIZE*8; //*8 since there are 8 bits in 1 byte 
		//(in each entry of bit map corresponds to a block and is 0 (unused) or 1 (used)
		//want bit map to fit into 1 block --> 1024 bytes --> can represnt 1024*8 blocks
...
void write_block_group_descriptor_table(int fd) {
	...
	block_group_descriptor.bg_used_dirs_count = NUM_DIR_USED;
...
void write_block_bitmap(int fd) {
	off_t off = BLOCK_OFFSET(BLOCK_BITMAP_BLOCKNO); //gets offset of bitmap bloco
	off = lseek(fd, off, SEEK_SET); //moves cursor to bitmap so we can write to there
	if (off == -1) {
		errno_exit("lseek");
	}
	unsigned char bits[BLOCK_SIZE] = {0x00};
	//first 23 bits should be one
	bits[0] = 0xff; //sets first 8 bit to 1
	bits[1] = 0xff; //sets next 8 bits to 1
	bits[2] = 0x7f; //sets next 7 bits to 1
	//since we only have 1024 blocks and block 0 = boot is not in bitmap
	//everything from 1024 to after is not used and should have 1s
	//1024/8 = 128
	bits[127] = 0x80;
	for (ssize_t i = 128; i< BLOCK_SIZE; i++){
		bits[i] = 0xff;
	}

	if (write(fd, bits, BLOCK_SIZE) != sizeof(bits)){
		errno_exit("write");
	}
}

void write_inode_bitmap(int fd) {
	off_t off = BLOCK_OFFSET(INODE_BITMAP_BLOCKNO); //gets offset of bitmap bloco
	off = lseek(fd, off, SEEK_SET); //moves cursor to bitmap so we can write to there
	if (off == -1) {
		errno_exit("lseek");
	}
	...
	//first 13 bits should be one
	bits[0] = 0xff; //sets first 8 bit to 1
	bits[1] = 0x1f; //sets next 5 bits to 1
	
	//since we only have 1024 blocks and block 0 = boot is not in bitmap
	//everything from 1024 to after is not used and should have 1s
	//128/8 = 16
	for (ssize_t i = 16; i< BLOCK_SIZE; i++){
		bits[i] = 0xff;
	}
...
void write_inode_table(int fd) {
	...
	root_inode.i_gid = 0;
	root_inode.i_links_count = 3; //3 there is a directory (lost+found) in root
...
void write_root_dir_block(int fd) {
	...
	struct ext2_dir_entry current_entry = {0}; //initialize all fields to 0
	dir_entry_set(current_entry, EXT2_ROOT_INO, "."); //sets field of struct
	dir_entry_write(current_entry, fd); //write entry to root
...
int main(int argc, char *argv[]) {
	...
	//open and set file content to all 0
	int fd = open("cs111-base.img", O_CREAT | O_WRONLY, 0666);
	if (fd == -1) {
		errno_exit("open");
	}

	if (ftruncate(fd, 0)) { //sets file to size 0
		errno_exit("ftruncate");
	}
	if (ftruncate(fd, NUM_BLOCKS * BLOCK_SIZE)) { //increases file to specified size (1Mb) and pads it with 0
		errno_exit("ftruncate");
	}
